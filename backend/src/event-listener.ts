...
const watchEvent = client.watchContractEvent({
  ...
  onLogs: (logs) => {
    try {
      if (logs[0])
        checkAndPushBlock(
          logs[0].blockNumber as bigint,
          logs[0].blockHash as string,
        );
...
async function checkAndPushBlock(blockNumber: bigint, blockHash: string) {
  ...
  if (!hash) numToHash.set(blockNumber, blockHash);
    ...
    while (found) {
      ...
      try {
        const block = await client.getBlock({
          blockNumber: currentBlock as BlockNumber,
        });
...
async function reprocessEventsFromBlock(fromBlock: bigint, toBlock: bigint) {
  try {
    ...
    for (const log of logs) {
      ...
      console.log(
        `Updated block ${log.blockNumber} with correct hash: ${log.blockHash}`,
      );
    }
